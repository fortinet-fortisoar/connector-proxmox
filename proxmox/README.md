...
### Inventory
- **List VMs** – All VMs on a node; optionally fetches full config and produces human‑friendly disk/interface summaries (`disksSummary`, `interfacesSummary`).  
...
For each operation there are **sample playbooks** in the collection `Sample - Proxmox - 1.0.0`. Token capabilities and permission requirements: `TOKEN_CAPABILITIES.md` in the project root.  

## Import into FortiSOAR

1. Package the folder as `.tgz`:
   ```bash
   tar -czvf "Proxmox.tgz" "proxmox/"
   ```
...
**2.0.4**
- Inventory connector hardened for FortiSOAR inventory sync:
  - `list_vms` / `list_containers` now always support `include_config` and correctly unwrap `config` from the Proxmox API response.
  - Added helper functions to build concise, human‑friendly **Disks** and **Interfaces** summaries for use in the `proxmox_inventory` module and VM Instances view.

**2.0.3**
- Fixed config handling for `list_vms` / `list_containers` when used by the "> Refresh Proxmox Inventory" playbook (unwrap `response.data`).

**2.0.0**
- Added inventory‑focused operations:
  - `get_vm_config`, `get_container_config` for direct config queries.
  - Extended output schema for `list_vms` / `list_containers` to expose `config`, `disksSummary`, `interfacesSummary`.
...
*Connector version 1.0.0 | Built for Proxmox VE 9.x*
